...
case $SYS_ID in
arch)
  pacman -Sy --needed --noconfirm kubectl >&2
  ;;
fedora)
...
EOF
  dnf install -y kubectl >&2
  ;;
debian | ubuntu)
  export DEBIAN_FRONTEND=noninteractive
  apt-get update >&2 && apt-get install -y apt-transport-https ca-certificates curl >&2
  ...
  apt-get update >&2 && apt-get install -y kubectl >&2
  ;;
*)
  while [[ ! -f kubectl ]]; do
