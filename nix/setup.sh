...
# check if dev-env is already installed in the profile
if nix profile list --json 2>/dev/null | jq -e '.elements["dev-env"]' &>/dev/null; then
  # upgrade the existing dev-env package (picks up scope changes + new nixpkgs)
  nix profile upgrade dev-env \
